    ...
    <div class="bg-white overflow-auto">
        <table class="min-w-full leading-normal">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="1000"> Sem nenhuma dúvida</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<nav aria-label="Page navigation py-12 " align="right">
    <ul class="inline-flex -space-x-px">
        @if ($supports->currentPage() > 1)
            <li>
                <a href="?page={{ $supports->currentPage() - 1 }}&status={{ request('status', 'P') }}" class="py-2 px-3 ml-0 leading-tight text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">Anterior</a>
            </li>
        @endif
        <li>
            <a href="#" aria-current="page" class="py-2 px-3 text-blue-600 bg-blue-50 border border-gray-300 hover:bg-blue-100 hover:text-blue-700">{{ $supports->currentPage() }}</a>
        </li>
        @if ($supports->currentPage() < $supports->lastPage())
            <li>
                <a href="?page={{ $supports->currentPage() + 1 }}&status={{ request('status', 'P') }}" class="py-2 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">Próxima</a>
            </li>
        @endif
    </ul>
</nav>
